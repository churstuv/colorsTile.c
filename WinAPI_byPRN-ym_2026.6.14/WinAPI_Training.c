    ...
    LPARAM lParam)          // IParam: 추가 메시지 정보
{
    switch (msg)
    {
        case WM_CREATE:
        {
            ...
            mciSendString(              // bgm 파일 불러오기: media coontrol interface
                L"open bgm.mp3 type mpegvideo alias bgm",       // command: bgm.mp3 파일 열기. 이 파일 별명 bgm으로 함.
                NULL,       // returnString
                0,          // returnLength
                NULL);      // callback
...
void DrawHelp(HDC hdc) 
{
    ...
    wchar_t text[100] =     // reQuest: 여기에 설명 적으시면 됩니다. ㅇㅇ
