...
void initBoard();
void printBoard();
int searchAndMatch(int startX, int startY);     // **판별 위해 colorMatched 반환함수로 변경
void shuffleBoard();
bool checkDeadlock();
void saveRanking(wchar_t* playerName, int score);
playerInfo* showRanking(int* count);
...
int searchAndMatch(int startX, int startY) {        // **int colorMatched 반환해서 효과음 적용
    // 여기에 사방 탐색 로직 작성 예정
    ...
    return colorMatched;        // **반환 가능한 함수로 변경.(맞췄는지 못 맞췄는지 판별용)
...
void saveRanking(wchar_t* playerName, int score) {
    FILE* fp = NULL;
    _wfopen_s(&fp, L"ranking.txt", L"a");
...
playerInfo* showRanking(int* count) {
    ...
    FILE* fp = NULL;
    _wfopen_s(&fp, L"ranking.txt", L"r");

    playerInfo temp;    // 동적메모리에 쓰일 count_player 먼저 계산용.

    while (fwscanf(
        fp,
        L"%ls %d",
        temp.name,
        &temp.score) == 2)     // 2: 성공적으로 읽어들인 항목 개수.
    {
        count_player++;
    ...
    */
    ...
    fclose(fp);
    // playerInfo player[100];     // **동적메모리 할당받아서 전부 다 집계할 예정.
    
    // 동적 할당의 장면
    playerInfo* player =
        malloc(sizeof(playerInfo) * count_player);  // 동적 메모리 할당.
    ...
    // 정렬하기
    for (int i = 0; i < count_player - 1; i++) {        // 오름차순 정렬(score)
